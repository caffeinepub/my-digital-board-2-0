...
export function StaffingBoard({ cards, onCardMove, isLocked, showToast }: StaffingBoardProps) {
  ...
  function onDrop(e: React.DragEvent, sectionKey: string) {
    if (isLocked) {
      showToast('Locked. Unlock to edit.');
      return;
    }
  ...
  return (
    ...
    >
      <style>{`
        @media (max-width: 1180px) { .staffing-board { grid-template-columns: repeat(3, 1fr) !important; } }
        @media (max-width: 820px)  { .staffing-board { grid-template-columns: repeat(2, 1fr) !important; } }
        @media (max-width: 560px)  { .staffing-board { grid-template-columns: 1fr !important; } }
      `}</style>

      {STAFFING_COLS.map((col) => {
        const colCount = col.sections.reduce(
          (sum, s) => sum + (cardsBySection[s.key]?.length || 0),
          0
        );
        return (
          <div
          ...
          >
            ...
            >
              {col.sections.map((sec) => {
                ...
                const sectionCards = cardsBySection[sec.key] || [];
                return (
                  <div
                    key={sec.key}
                    style={{
...
function StaffCardItem({ card, isLocked, isDragging, onDragStart, onDragEnd }: StaffCardItemProps) {
  return (
    <div
      draggable={!isLocked}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          >
            {card.personName}{' '}
            <span style={{ fontWeight: 400, color: 'rgba(255,255,255,0.5)', fontSize: '12px' }}>
              ({card.login})
            </span>
          </p>
          <div
