...
export default function App() {
  ...
  const [isLocked, setIsLocked] = useState(() => localStorage.getItem(LOCK_KEY) === '1');
  ...
  const [localState, setLocalState] = useState<AppBoardState | null>(null);

  // Sync local state from backend when loaded
  useEffect(() => {
    if (boardState && !localState) {
      setLocalState(boardState);
    }
  }, [boardState, localState]);

  const state = localState ?? boardState ?? defaultAppState();
  ...
  const appId = encodeURIComponent(
    typeof window !== 'undefined' ? window.location.hostname : 'my-digital-board'
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-start', minWidth: 0 }}>
          <select
            className="board-toggle"
            value={activeBoard}
            onChange={(e) => setActiveBoard(e.target.value as 'amazon' | 'snhu')}
            title="Select board"
            style={{
              width: '360px',
              maxWidth: '100%',
          ...
          >
            <option value="amazon">Amazon Workplace: Demorians Department</option>
            <option value="snhu">Southern New Hampshire University (SNHU)</option>
          </select>

          <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.55)', display: 'flex', gap: '14px', flexWrap: 'wrap', alignItems: 'center' }}>
            <span>
              <strong style={{ color: 'rgba(255,255,255,0.7)' }}>Last Updated:</strong>{' '}
              {lastUpdated}
            </span>
            {isLoading && (
        ...
        >
          ...
          >
            ...
            <span style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{liveTime}</span>
          </div>
        </div>
      ...
      >
        <span>© {new Date().getFullYear()} My Digital Board 2.0 · Built with</span>
